...
def TranzilaAPI(EndPoint, Terminal, Body, BaseURL=env("TRANZILA_BASEURL")):
    ...
    PublicKey = env(
        ...
        if Terminal == "ultranet1atok"
            ...
            "TRANZILA_2P_PUBLIC_KEY"
            if Terminal != "ultranettest"
            else "TRANZILA_TEST_PUBLIC_KEY"
        )
    )
    PrivateKey = env(
        ...
        if Terminal == "ultranet1atok"
            ...
            "TRANZILA_2P_PRIVATE_KEY"
            if Terminal != "ultranettest"
            else "TRANZILA_TEST_PRIVATE_KEY"
        )
    )
    ...
    ResponseCode = Result.get("transaction_result", {}).get(
...
def GetTransactions(Terminal):
    Body = {
        ...
        "transaction_end_date": "2026-01-30",
        "page": 1,
        "page_results": 100,
    }
...
def GetTransactionByID(ClientID, Terminal):
    ...
    ClientTransactions = [
        ...
        if str(tran.get("credit_card_owner_id") or "").strip() == client_id
    ]
...
def VerifyCreditCard(
    ExpDate, CVV, CardNumber, ClientInstance, IDNumber, Terminal="ultranettest"
):
    ...
    Body = {
        ...
        "client": {
            ...
            "email": (
                ...
                if ClientInstance.Email != "" and ClientInstance.Email is not None
                else None
            ),
        },
        "items": [{"name": "כללי", "type": "I", "unit_price": 1, "units_number": 1}],
        "user_defined_fields": [
...
def Refund(
    ...
    TransactionID,
    AuthNumber,
    Terminal="ultranettest",
):
    return CreatePaymentRequests(
        ExpDate,
        CVV,
        CardNumber,
        ClientInstance,
...
def CreatePaymentRequests(
    ...
    if transaction_type == "credit" or transaction_type == "cancel":
        Body["reference_txn_id"] = (
            TransactionID  # this is the id of the transaction that we want to refund
        )
        Body["authorization_number"] = (
            AuthNumber  # this is the auth number of the transaction
        )
    print(Body)
    return TranzilaAPI(EndPoint, Terminal, Body, BaseURL)
